...
{
  BinaryNodeX<Comparable> *t = this->root;
  
  while(t)
  {
    if (possibleAncestor == t->element)
      break;
    else if(possibleAncestor > t->element)
      t = t->right;
    else if(possibleAncestor < t->element)
      t = t->left;
  }
...
int BinarySearchTreeX<Comparable>::highestFull(BinaryNodeX<Comparable> *t) const
{
  ...
  // int left = highestFull(t->left); 
  // int right = highestFull(t->right);

  // if (t->left && t->right) // current node is at a full node
  //   return 0;
  // if (left == -1 && right == -1)
...
int BinarySearchTreeX<Comparable>::deepestFull(BinaryNodeX<Comparable> *t) const
{
  if (!t->left || !t->right)
    ...
    return 1 + max(deepestFull(t->left), deepestFull(t->right));
}
